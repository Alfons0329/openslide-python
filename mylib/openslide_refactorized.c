...
static bool read_region(openslide_t *osr,
  ...
  // printf("\t\t[Openslide.C TIME LOG]: read_region preprocessing time %.2lf s\n", t_cpu);
  ...
  // printf("\t\t[Openslide.C TIME LOG]: read_region paint_region time %.2lf s\n", t_cpu);
  ...
  // gettimeofday(&end, NULL);
  // t_cpu = (double)(end.tv_usec - begin.tv_usec) / 1000000.0 + (double)(end.tv_sec - begin.tv_sec);
  // printf("\t\t[Openslide.C TIME LOG]: read_region postprocessing time %.2lf s\n", t_cpu);
  return success;
}
...
void openslide_read_region(openslide_t *osr,
  ...
  const uint32_t thread_cnt = omp_get_max_threads();
  const uint32_t target_width = w, target_height = h;
